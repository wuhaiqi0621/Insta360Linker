...
## 2026-08-17 - SwiftUI feature parity and 12× camera zoom

- Read the repository instructions and latest development records, fetched GitHub, compared both histories and safely fast-forwarded local `main` from `3a2f5ab` to `b0a0847`. Reviewed all six incoming commits before editing.
- Preserved the user's pre-existing unstaged changes in `.github/workflows/build-android.yml` and `reverse_apk/tools/ashield_unpack_cluster_disasm.txt`; neither file was modified, staged or included in this work.
- Audited the shared Web/Rust frontend against the pure SwiftUI macOS frontend command by command. The missing native adaptations were media preview and LRV pairing, media sorting/density, complete video profiles, correct gimbal values, recording feedback and Mic Pro GATT writing.
- Kept the Android-only transfer task engine, JNI pause/resume/cancellation controls and MediaStore publishing Android-specific. The macOS backend uses a separate synchronous process bridge, while Windows virtual-camera/UCD2 diagnostic controls remain intentionally unavailable in the native macOS product.
- Added native camera-media preview preparation to the Rust macOS helper. Authenticated camera files are validated, resumably cached under the application cache and opened in a SwiftUI sheet using AppKit image decoding or the system AVKit player.
- Cached LRV/INSV preview containers with an `.mp4` presentation extension so AVKit recognizes the camera's MP4-compatible video content instead of depending on proprietary filename-extension inference.
- Added Web-equivalent LRV behavior to SwiftUI: linked `LRV_` companions are hidden from the logical gallery, used for original-video thumbnails and playback, and deleted together with the corresponding `VID_` original.
- Added native newest/oldest sorting, three gallery density levels, click-to-preview, explicit selection controls, context-menu preview/download/delete/watermark actions and failed-download selection retention.
- Replaced the incomplete Swift recording-format picker with all ten Luna Ultra formats and their exact supported frame-rate tables, including 8K 2.35:1, vertical formats and 1080p 240/200 fps. Mode/profile responses now update the visible Swift state.
- Corrected the Swift gimbal pad to send the captured UI vectors (`up 0,-72`, `down 0,72`, `left -72,0`, `right 72,0`) and made the center button send the protocol stop/zero command. Limited the native speed control to the three hardware-supported levels.
- Added a native recording elapsed-time indicator and delayed media refresh after a photo or completed recording, while preventing concurrent preview, zoom and gimbal actions from being submitted through busy controls.
- Added structured Mic Pro GATT characteristic rows, automatic selection of the first writable characteristic, validated hexadecimal input and native `write_ble` execution.
- Corrected the shared BLE writer to choose `WithResponse` or `WithoutResponse` from the selected characteristic's real properties instead of forcing response writes onto every writable Mic Pro characteristic; added a focused regression test.
- Updated Luna Ultra zoom validation from the old discrete values to the camera's full `1.0×...12.0×` range at exact `0.1×` increments. The SwiftUI control is a native slider, and the shared Windows/Android Web frontend now uses the same 1-12 range slider and one-decimal display.
- Added exact protocol regression vectors for `7.5×` and `12.0×`, plus rejection coverage for out-of-range, non-tenth and non-finite values.
- Updated `README.md` to document the unified 12× slider, complete video profiles, native media preview/LRV behavior and Mic Pro GATT support.
- Added the previously missing `*.swift text eol=crlf` repository attribute and normalized every modified text file to UTF-8 CRLF, matching `AGENTS.md` instead of leaving Swift sources dependent on Git's platform heuristic.
- Passed the standalone Swift 6.4 compile targeting macOS 26 with the macOS 27 SDK, shared inline-JavaScript syntax validation using the bundled Node.js runtime, `cargo fmt --all --check` and `cargo test --locked --bin Insta360Linker` (`49 passed`, `0 failed`, `2` physical-hardware/external-input tests explicitly ignored).
- Rebuilt the release app, smoked the packaged native JSON helper with the official watermark catalog command, passed strict deep code-signature verification, Info.plist validation and dependency inspection. The frontend links SwiftUI/AppKit/AVKit and does not link WebKit.
- Launched the rebuilt app and verified its native accessibility tree exposes the expected sidebar, toolbar, connection group and unavailable state. Physical Luna Ultra Wi-Fi, live preview, 12× lens movement and Mic Pro GATT operations could not be exercised without the devices and remain explicit hardware verification items.
- Installed the verified bundle at `/Applications/Insta360Linker.app`. Final installed SHA-256 values: SwiftUI frontend `01bdf2cf82cb5211637efb1003862cf3f3c58378ab7d03baad1c8111b338d326`; Rust helper `e74d32719b5a8b1bc0a5dcd94d41969bf7b337c6cb48c31ff0aef98915489a41`.
- Android was not rebuilt locally because this Mac has no Java runtime; the shared Web zoom script passed syntax validation and the repository GitHub Actions Android workflow remains the build path.
- The first push attempt exposed a stale global GitHub credential-helper path under `/tmp`; no repository data was sent by that failed attempt. Installed official GitHub CLI `2.97.0` under the user's local application directory and verified its macOS ARM64 archive against the release checksum `a58b8fd77b417a38f47a0b54d1370c59b0fcdb324ccc9ca002b0998f7c4c999e` before use.
- Confirmed the existing keyring login belongs to `wuhaiqi0621` with repository scope, replaced the dead temporary helper path with the stable verified CLI path and configured Git HTTPS authentication without printing or storing plaintext credentials in the repository.
- Committed the verified SwiftUI parity implementation as `1f3b93c` (`feat: complete SwiftUI camera feature parity`) and pushed it successfully to GitHub `origin/main`. The user's pre-existing unstaged Android workflow and reverse-engineering text changes remained local and were not included in that commit.

## 2026-08-30 - Reproducible three-platform release packaging

- Re-read the repository instructions and latest development records, fetched GitHub before editing, and confirmed local `HEAD` and `origin/main` both pointed to `754d2bd5baf363531efc8fcf80e48930d5760b1e`; no pull was required.
- Preserved the user's pre-existing unstaged changes in `.github/workflows/build-android.yml` and `reverse_apk/tools/ashield_unpack_cluster_disasm.txt`. The release work does not overwrite the disassembly text and does not depend on the legacy Android-only workflow.
- Audited the current distribution paths and confirmed that the repository had no GitHub Releases, no Windows CI package, only a debug-oriented Android workflow, and no unified checksum-gated macOS/Windows/Android release process. Cargo, Android and macOS metadata consistently report version `0.1.0`.
- Added `Build Release Packages`, a tag/manual GitHub Actions workflow for macOS ARM64, Windows x64 and Android ARM64. The macOS job uses the Xcode 27 runner and verifies the native SwiftUI bundle, signature, architectures, official watermarks, icon catalog and absence of a WebKit frontend dependency.
- Made the Windows job build both the desktop executable and virtual-camera DLL, verify the independently downloaded FFmpeg archive against its published SHA-256 entry, and package FFmpeg plus the complete official watermark resource tree next to the application.
- Made the Android job compile the renamed `libinsta360_linker.so`, run Java unit tests and Android Lint, verify the installable test-signed APK and assert that the JNI library, shared Web UI and official watermarks are present. The package name states that it is test-signed instead of presenting it as a store-signed binary.
- Configured `v*` tag builds to publish a GitHub prerelease only after all three platform jobs succeed. Each package has a separate SHA-256 file, and the release notes explicitly describe the macOS ad-hoc and Android test-signing limitations.
- Updated the root and Android build documentation for the unified workflow and corrected the stale Android capability note that still claimed HEVC monitoring was unavailable after the MediaCodec implementation had already landed.
- Changed the macOS release build to use Cargo's lockfile and assemble into a hidden staging bundle before replacing the previous app. This makes a compiler, copy or signing failure leave an existing successful package intact.
- The first local macOS packaging attempt completed the Rust release build but exposed a host Command Line Tools mismatch: the selected SDK was built with Swift `6.4.0.31.4` while the selected compiler was `6.4.0.30.4`. That attempt removed the ignored `dist/Insta360Linker.app` generated by an earlier build; the new transactional staging change prevents that loss on subsequent failures. The installed `/Applications/Insta360Linker.app` was not modified.
- Passed YAML parsing, shell syntax validation, focused whitespace checks, secret-pattern scanning, `cargo fmt --all -- --check`, and the complete locked Rust suite with the required loopback-test permission (`49 passed`, `0 failed`, `2` physical-hardware/external-input tests ignored). Local Android compilation remains unavailable because this Mac has no Java runtime; Windows and authoritative Xcode 27/macOS packaging are delegated to the checked-in GitHub Actions jobs.
- Pushed the initial release workflow in commit `d89847f` after extending the existing GitHub CLI authorization with the required `workflow` scope. No password or token was written to the repository.
- Manual validation run `33293432492` exposed that shell version extraction retained Cargo.toml's CR byte: the macOS plist comparison and Android Gradle comparison failed before either platform compiled. The Windows job had entered its first cold Rust build, but the overall run could no longer succeed, so it was cancelled to avoid wasting runner time.
- Corrected both shell jobs to remove CR from the parsed Cargo version, then reproduced the exact macOS plist and Android `versionName` comparisons locally; both now resolve to `0.1.0` and pass before the workflow is retried.
- Manual validation run `33293648295` then passed all jobs: Xcode 27 macOS ARM64 in 2 minutes 46 seconds, Android ARM64 JNI/tests/Lint/APK validation in 3 minutes 15 seconds, and Windows x64 tests/build/runtime packaging in 12 minutes.
- Downloaded all three artifacts to a temporary verification directory. The macOS archive checksum and ZIP integrity passed, while independent Unix verification exposed that PowerShell had written the Windows checksum file with CRLF and left a CR attached to the listed filename.
- Changed the Windows checksum writer to emit an explicit ASCII/LF record so the same `.sha256` file works with PowerShell, GNU `sha256sum` and macOS `shasum -c` instead of only the generating Windows environment.
- Final manual run `33294223172` rebuilt commit `6ba5904` and passed all three jobs again. Downloaded all outputs, verified all three provided SHA-256 records directly on macOS, tested every ZIP/APK container, and confirmed the Windows package contains the EXE, virtual-camera DLL, FFmpeg executable and official Luna watermark resource.
- Tagged exact verified commit `6ba5904c8c7e5cd14cc440db4034dff70e93c4bf` as `v0.1.0`. Tag run `33294721815` again passed the macOS, Android and Windows build/package jobs, but its five-second publish job lacked both a checkout and an explicit repository argument, so GitHub CLI could not infer a repository from the runner directory.
- Added the explicit `$GITHUB_REPOSITORY` argument to release creation so future tag publishing does not depend on a checked-out `.git` directory. The already verified `v0.1.0` tag artifacts are published directly for this release instead of recompiling the unchanged source a fourth time.
- Published the verified tag artifacts as the `Insta360Linker v0.1.0` GitHub prerelease at `https://github.com/wuhaiqi0621/Insta360Linker/releases/tag/v0.1.0`. GitHub reports all six assets (three packages plus their checksum records) in the uploaded state.
- Final tag-build package SHA-256 values are: macOS ARM64 ZIP `bbd8e01b516ccfd18fe611768e2fc3b09ab6022603f24cdd24fa1ba2118a87c7`; Windows x64 ZIP `89b45d3960a8239648c19e097dbb059454cd9a53f9a43ad319c94cf2494068a5`; Android ARM64 test-signed APK `0c4ece2eaaad7df1c6c2554e65c4dc2762ed6cac57f9ff30a7d26c395383b71b`.
- Copied the six final artifacts to ignored local directory `dist/releases/v0.1.0` and re-ran each supplied checksum record there; macOS, Windows and Android all returned `OK`. These are byte-identical to the GitHub Release assets.

## 2026-08-30 - Windows single-file release

- Fetched GitHub before editing and confirmed local `HEAD` and `origin/main` both pointed to `2ba2a2a00dbf92d5c03f1b601ab0c82b7c2e28a8`; no pull was required. Re-read the complete repository development log and preserved the user's existing unstaged `.github/workflows/build-android.yml` and reverse-engineering text changes.
- Changed the supported Windows distribution format from a ZIP directory to one `Insta360Linker.exe`. Official watermark PNGs are compiled directly into the executable; runtime lookups prefer those embedded bytes, so removing an external `assets` directory no longer causes missing-official-resource errors.
- Added build-time embedding for the independently verified FFmpeg executable, its GPLv3 license and the Windows virtual-camera DLL. The EXE exposes the embedded license through `--third-party-licenses`.
- Added hash-addressed, atomic runtime materialization under `%LOCALAPPDATA%\\Insta360Linker\\Runtime\\<version>`. FFmpeg and the COM DLL are released only when needed, verified by length and FNV-1a content hash, reused after successful validation and never require the user to keep files beside the EXE.
- Kept ordinary development builds compatible with external resources, while making the Release workflow perform a two-pass Windows build: build the DLL first, then compile the verified DLL/FFmpeg/license plus every official watermark PNG into the final executable.
- Added the `--verify-single-file` non-UI self-test and made GitHub Actions run it before upload. Packaging now rejects an EXE that cannot materialize its embedded resources or is too small to contain the verified FFmpeg and DLL, and uploads only the EXE plus its SHA-256 record.
- Updated Windows usage/build documentation and bumped Cargo, macOS and Android versions consistently to `0.1.1` (`CFBundleVersion` and Android `versionCode` are now `2`) for the new release.
- Passed YAML parsing, `cargo fmt --all -- --check`, `cargo check --locked --bin Insta360Linker`, focused whitespace validation and the complete locked local suite (`49 passed`, `0 failed`, `2` physical-hardware/external-input tests ignored). Windows-specific embedding and materialization remain assigned to the Windows GitHub Actions runner for executable verification.
- Manual validation run `33296036622` passed macOS ARM64 in 2 minutes 51 seconds and Android ARM64 in 2 minutes 19 seconds. The Windows source tests, verified FFmpeg download and seven-minute DLL build passed, but the workflow correctly stopped before compiling the final EXE because the BtbN binary archive does not contain the GPLv3 license filename the new embedding gate required.
- Kept the license gate and changed its source to FFmpeg's official `n8.0` tag. The workflow now downloads `COPYING.GPLv3` separately and requires SHA-256 `8ceb4b9ee5adedde47b31e975c1d90c73ad27b6b165a1dcd80c7c545eb65b903` before embedding it; an absent or altered license still fails the release.
- Manual validation run `33296553165` passed the official license gate, built the final single EXE and again passed macOS/Android. Its runtime self-test successfully materialized FFmpeg but rejected the virtual-camera DLL: Cargo's second package build rewrote `target/release/insta360_linker.dll` while the generated embedding metadata referenced that mutable path, creating a content/hash race.
- Removed that race by copying the first-pass DLL to immutable `build/embed/Insta360LinkerVirtualCamera.dll` before setting the embedding environment. Both build-time hashing/embedding and package-size verification now use the unchanged staged DLL rather than Cargo's mutable target output.
- Manual validation run `33297086759` passed all three targets: Windows x64 in 12 minutes 24 seconds, macOS ARM64 in 3 minutes 2 seconds and Android ARM64 in 2 minutes 24 seconds. The Windows runner built a 157,954,048-byte standalone EXE, successfully materialized and content-verified its embedded FFmpeg and virtual-camera DLL, found the required embedded official watermark set and uploaded only the EXE plus its checksum record.
- Downloaded the complete manual-run artifacts and independently verified every supplied SHA-256 record, the macOS ZIP and Android APK container integrity, the Windows PE32+ x64 format and the embedded GPLv3 license text before tagging.
- Tagged exact verified commit `f7b205967394fe65522d86c5ec1610fe4c5ac76b` as `v0.1.1`. Tag run `33297642996` passed macOS in 2 minutes 42 seconds, Android in 2 minutes 29 seconds, Windows single-EXE build/self-test/upload in 11 minutes 17 seconds and the automatic GitHub prerelease publication job in 17 seconds.
- Published `Insta360Linker v0.1.1` at `https://github.com/wuhaiqi0621/Insta360Linker/releases/tag/v0.1.1` with six uploaded assets. Final package SHA-256 values are: Windows x64 single EXE `974329c3b3f0e985c1930efe25e4e51d273fdc897880fe3d6200c1452be810b1`; macOS ARM64 ZIP `71ed805f826602eb10fb44b54e63292e424eeed0c35e0585691ead2dcad0b1d7`; Android ARM64 test-signed APK `2a16e0aa0d16a3b88e4f2af9a04bfc53f545ce3af0a556d6a31f0cd54654263d`.
- Downloaded the final tag-built assets, reverified all checksum records and archive containers, confirmed the EXE is x64 PE and still contains the GPLv3 text, then copied all six files to ignored local directory `dist/releases/v0.1.1`.
